"""
SQL Database module - SQL database connection và query utilities.
...
class SQLConnector:
    ...
    def __init__(
        ...
        Args:
            db_type: Loại database: "postgres" hoặc "mysql"
            host: Host của database (mặc định từ env)
            port: Port của database (mặc định từ env)
            database: Tên database (mặc định từ env)
            user: Username (mặc định từ env)
            password: Password (mặc định từ env)
        """
        ...
        if db_type.lower() == "postgres":
            ...
            raise ValueError(
                ...
                f"Chỉ hỗ trợ 'postgres' hoặc 'mysql'"
            )
        ...
        #     logger.info(
        #         f"Đã kết nối thành công đến {self.db_type} database: "
        ...
        # Tạm thời set None để tránh lỗi khi gọi các hàm
        self.db = None
        self.sql_tool = None
        logger.warning(
            f"SQL database connection đã bị tắt. "
            f"Bỏ comment phần kết nối trong SQLConnector.__init__ để sử dụng lại."
        )
    ...
    def execute_query(self, query: str) -> Any:
        ...
        Args:
            query: Câu SQL query cần thực thi
        ...
        if self.sql_tool is None:
            raise RuntimeError(
                "SQL database connection chưa được khởi tạo. "
                "Vui lòng bỏ comment phần kết nối trong SQLConnector.__init__ để sử dụng."
            )
        try:
    ...
    def get_all_schemas(self, table_names: Optional[List[str]] = None) -> str:
        ...
        Args:
            table_names: Danh sách tên bảng (None để lấy tất cả)
...
def create_sql_connector(
    ...
    **kwargs
) -> Optional[SQLConnector]:
    """
    Tạo SQLConnector từ environment variables.
    
    Args:
        db_type: Loại database: "postgres" hoặc "mysql" (mặc định: tự động detect)
        **kwargs: Các tham số override cho connection
    
    Returns:
        SQLConnector instance hoặc None nếu không có cấu hình
    """
    ...
    if db_type is None:
        if os.getenv("POSTGRES_HOST") or settings.postgres_host:
            ...
            logger.warning(
                "Không tìm thấy cấu hình database trong environment variables"
            )
            return None
    
    try:
        connector = SQLConnector(db_type=db_type, **kwargs)
        return connector
    except Exception as e:
...
def get_sql_connector() -> Optional[SQLConnector]:
    ...
    Returns:
        SQLConnector instance hoặc None
    """
...
def init_sql_connector(
    db_type: Optional[str] = None, **kwargs
) -> Optional[SQLConnector]:
    """
    ...
    Returns:
        SQLConnector instance hoặc None nếu không thể tạo
    """
    global _sql_connector
    if _sql_connector is None:
